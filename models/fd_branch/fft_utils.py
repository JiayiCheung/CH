...
class ComplexLinear3D(nn.Module):
    """
    复数版的 1x1x1 Linear/Conv 操作，用于频域张量通道混合
    - 输入: X [B, Cin, D, H, W] (complex)
    - 输出: Y [B, Cout, D, H, W] (complex)
    """
    def __init__(self, Cin, Cout):
        super().__init__()
        # 拆成 real/imag 两组参数
        self.weight_real = nn.Parameter(torch.randn(Cout, Cin) * 0.02)
        self.weight_imag = nn.Parameter(torch.randn(Cout, Cin) * 0.02)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: complex tensor, shape [B, Cin, D, H, W]
        """
        xr, xi = x.real, x.imag  # [B,Cin,D,H,W]
