...
def calculate_hausdorff_distance(pred, target, percentile=95):
	...
	# 计算第percentile百分位数距离
	pred_to_target_percentile = np.percentile(pred_to_target, percentile)
	target_to_pred_percentile = np.percentile(target_to_pred, percentile)
	
	# Hausdorff距离是两个方向距离的最大值
	hd = max(pred_to_target_percentile, target_to_pred_percentile)
...
def calculate_all_metrics(pred, target):
	...
	metrics = {
		'dice': calculate_dice(pred, target).item(),
		'iou': calculate_iou(pred, target).item(),
		'precision': calculate_precision(pred, target).item(),
		'recall': calculate_recall(pred, target).item()
	}
	
	# Hausdorff距离和ASSD需要NumPy数组
	pred_np = pred.cpu().numpy()
	target_np = target.cpu().numpy()
	
	metrics['hausdorff'] = calculate_hausdorff_distance(pred_np, target_np)
	metrics['assd'] = calculate_assd(pred_np, target_np)
