...
class MMapManager:
		...
		with filelock.FileLock(str(lock_path)):
			if map_path.exists():
				...
				try:
					...
					logger.debug(f"Loaded existing mmap for {case_id}, shape: {shape}")
				except Exception as e:
					# 文件可能损坏，重新创建
					logger.warning(f"Error loading mmap for {case_id}: {e}. Recreating...")
					if map_path.exists():
	...
	def _create_new(self, path, shape, dtype, initial_value):
		...
		mmap_array.flush()
		logger.debug(f"Created new mmap at {path}, shape: {shape}")
		return mmap_array
	...
	def sync_to_disk(self, case_id=None):
		...
		if case_id is not None:
			if case_id in self.open_maps:
				...
				logger.debug(f"Synced mmap for {case_id} to disk")
		else:
			# 同步所有打开的映射
			for case_id, mmap_array in self.open_maps.items():
				mmap_array.flush()
			logger.debug(f"Synced all {len(self.open_maps)} mmaps to disk")
	...
	def close(self, case_id=None):
		...
		if case_id is not None:
			if case_id in self.open_maps:
				...
				logger.debug(f"Closed mmap for {case_id}")
		else:
			# 关闭所有打开的映射
			self.sync_to_disk()
			self.open_maps.clear()
			logger.debug("Closed all mmaps")